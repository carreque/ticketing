...
- **Server-generated fields are never client-settable:** `id` (ULID), `createdAt` (ISO-8601 UTC), `status` (always `"open"` on create). `Ticket` uses `extra="forbid"` so a client-supplied `status` is a 400. Domains: `priority ∈ {low,medium,high,critical}`, `status ∈ {open,in_progress,resolved}`.
